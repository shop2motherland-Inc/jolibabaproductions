...
class HomeController extends FrontController
{
	...
	{
		$data1 = '';
		if (Session::get("getProductscat")) {
			// print_r(Session::get("getProductscat"));
			$data1 = Post::where("category_id", Session::get("getProductscat")+1)->get();
			// dd($data1);
		}
		else
		{
		
			// $getcategory=Category::first()->id;
			// dd($getcategory);
			$data1 = Post::where("category_id", 2)->get();
			// dd($data1);
		}
		view()->share('Product', $data1);
		// Get the default Max. Items
		$maxItems = null;
		if (isset($value['max_items'])) {
			$maxItems = (int)$value['max_items'];
		...
		if (isset($value['cat_display_type']) && in_array($value['cat_display_type'], ['cc_normal_list', 'cc_normal_list_s'])) {
			...
			// 	$data1=Post::where("category_id",request()->get('category'))->get();
	...
	public function getSubCategoires($id)
	{
		$getSubCategoires=Category::where("parent_id",$id)->where("active",1)->get();
		return $getSubCategoires;
	}
}
