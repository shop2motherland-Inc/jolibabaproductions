		...
		<div class="row m-0">
			<div class="col-12 px-1 py-sm-1 bg-primary rounded">
				<div class="row gx-1 gy-1">
						...
						<select name="c" id="catSearch" class="form-control selecter">
							...
							@if (isset($rootCats) && $rootCats->count() > 0)
								@foreach ($rootCats as $itemCat)
									<option {{ ($qCategory == $itemCat->id) ? ' selected="selected"' : '' }} value="{{ $itemCat->id }}" data-name="{{$itemCat->name}}">
										{{ $itemCat->name }}
									</option>
								@endforeach
							@endif
					...
					<div class="col-xl-7 col-md-4 col-sm-12 col-12">
						<input name="q" class="form-control keyword" type="text" placeholder="{{ 'Search in ' . $cat->name }}" value="{{ $keywords }}">
					</div>
...
@section('after_scripts')
	...
	<script>
		$('#catSearch').on('change', function () {
			...
			if (categoryName)
				$('[name=q]').attr('placeholder', 'Search in ' + categoryName);
			else
			$('[name=q]').attr('placeholder', 'Search');
		})
	</script>
@endsection
