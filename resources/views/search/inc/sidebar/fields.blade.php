@if (isset($customFields) and $customFields->count() > 0)
	<form id="cfForm" role="form" class="form" action="{{ request()->url() }}" method="GET">
		...
		@foreach($customFields as $field)
			...
			// Field Query String
			$fieldQueryString = '<input type="hidden" id="cf' . $field->id . 'QueryString" value="' . \App\Helpers\Arr::query(request()->except(['page', $fieldId])) . '">';
			...
			@if ($field->type == 'checkbox')
				...
				<div class="block-title has-arrow sidebar-header">
					<h5>
						<span class="fw-bold"><a href="#">{{ $field->name }}</a></span> {!! $clearFilterBtn !!}
					</h5>
				</div>
				<div class="block-content list-filter">
						...
						<div class="filter-content"{!! $cmFieldStyle !!}>
							@foreach ($field->options as $option)
								...
								$fieldQueryString = '<input type="hidden" id="cf' . $field->id . $option->id . 'QueryString"
									value="' . \App\Helpers\Arr::query(request()->except(['page', $fieldId . '.' . $option->id])) . '">';
								?>
								<div class="form-check form-switch">
