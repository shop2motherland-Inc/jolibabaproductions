            ...
            <div class="navbar-identity p-sm-0">
                ...
                <a href="{{ url('/') }}" class="navbar-brand logo logo-title">
                    <img src="{{ imgUrl(config('settings.app.logo'), 'logo') }}"
                        ...
                        data-bs-toggle="tooltip" title="{!! isset($logoLabel) ? $logoLabel : '' !!}" />
                </a>
                {{-- Toggle Nav (Mobile) --}}
                ...
                @if (isset($multiCountriesIsEnabled) && $multiCountriesIsEnabled)
                    @if (!empty(config('country.icode')))
                        @if (file_exists(public_path() . '/images/flags/24/' . config('country.icode') . '.png'))
                            <button class="flag-menu country-flag d-block d-md-none btn btn-default hidden float-end"
           ...
           <span class="caret d-block float-end mt-3 mx-1 d-lg-block d-md-none"></span>
                                ...
                                <li class="nav-item dropdown lang-menu no-arrow open-on-hover">
                                    <a href="#" class="dropdown-toggle " data-bs-toggle="dropdown"
                                        id="langDropdown" style="padding:0px">
                                        <img class="flag-icon " style="width: 2.3rem;"
                                            src="{{ url('images/flags/32/' . config('country.icode') . '.png') . getPictureVersion() }}"
                                            alt="{{ config('country.name') }}">
                                        <i class="bi bi-chevron-down"></i>
                                    </a>
                                    <ul id="langDropdownItems"
                                        class="dropdown-menu dropdown-menu-end user-menu shadow-sm" role="menu"
                                        aria-labelledby="langDropdown">
                                        @if (isset($countries))
                                            @foreach ($countries as $code => $country)
                                                <div class="col mb-1 dropdown-item">
                                                    <?php
                            ...
                            <ul id="userMenuDropdown" class="dropdown-menu user-menu shadow-sm">
                                @if (isset($userMenu) && !empty($userMenu))
                                    ...
                                    @foreach ($userMenu as $key => $value)
                                        ...
                                        @endif
                                        <li
                                            class="dropdown-item{{ $value['path'] === $currentPath ? ' active' : '' }}">
                                            <a href="{{ $value['url'] }}">
